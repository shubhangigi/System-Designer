...
function api(method, path, summary, serviceId) {
    return {
        method,
        path,
        summary,
...
function node(id, name, type, responsibility, technology, position, extra = {}) {
    return {
        id,
        name,
        type,
...
export function createArchitecture(input, analysis) {
    ...
    if (needsCatalog) {
        ...
        edges.push({ id: 'api-gateway-catalog-service', source: 'api-gateway', target: 'catalog-service', type: 'sync', protocol: 'HTTP', purpose: 'Catalog API routing' }, { id: 'catalog-service-postgres', source: 'catalog-service', target: 'postgres', type: 'db', protocol: 'SQL', purpose: 'Restaurants and menu items' });
    }
    if (needsPayments) {
        nodes.push(node('payment-service', 'Payment Service', 'service', 'Creates payment intents, verifies callbacks, and isolates payment provider logic.', 'Node.js service module', { x: 560, y: 220 }, {
        ...
        edges.push({ id: 'order-service-payment-service', source: 'order-service', target: 'payment-service', type: 'sync', protocol: 'HTTP', purpose: 'Payment authorization' }, { id: 'payment-service-payment-provider', source: 'payment-service', target: 'payment-provider', type: 'external', protocol: 'HTTPS', purpose: 'Provider payment API' });
    }
    if (needsDelivery) {
        ...
        edges.push({ id: 'order-service-delivery-service', source: 'order-service', target: 'delivery-service', type: 'sync', protocol: 'HTTP', purpose: 'Delivery creation after order placement' }, { id: 'delivery-service-postgres', source: 'delivery-service', target: 'postgres', type: 'db', protocol: 'SQL', purpose: 'Delivery state' });
    }
    ...
    if (needsCatalog) {
        tables.push({ name: 'restaurants', columns: [col('id', 'uuid', { primaryKey: true }), col('name', 'text'), col('status', 'text')], indexes: ['status'] }, { name: 'menu_items', columns: [col('id', 'uuid', { primaryKey: true }), col('restaurant_id', 'uuid', { references: 'restaurants.id' }), col('name', 'text'), col('price_cents', 'integer')], indexes: ['restaurant_id'] });
    }
    if (needsDelivery) {
...
export function proposeArchitectureChange(model, instruction) {
    ...
    if (lower.includes('mongodb')) {
        return {
            ...
            patch: { kind: 'replace-database', technology: 'MongoDB' },
        };
    }
    return {
        ...
        patch: { kind: 'note', note: instruction },
    };
}
export function applyArchitectureChange(model, change) {
